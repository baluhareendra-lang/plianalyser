...
try:
    raw = read_excel_files(current_files)
    data = prepare(raw)
...
if data.empty:
    st.error("No valid records found in the uploaded file(s).")
    st.stop()
...
# ==============================================================================
    ...
    # 6. PERIOD COMPARISON SCREEN
        ...
        if previous_files:
            try:
                prev_raw = read_excel_files(previous_files)
                prev = prepare(prev_raw)
                prev = prev.merge(active_ptype_map, left_on="_RAW_POLICY_TYPE", right_on="RAW_POLICY_TYPE", how="left")
                prev["_SCHEME"] = prev["SCHEME"].fillna("PLI")
                prev_office = aggregate_by_scheme(prev)
